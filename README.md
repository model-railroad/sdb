...
- Open `sdb/CMakeLists.txt` in Notepad++ or any editor and validate that the top level variables
  for `ARDUINO_DIR` and `XTENSA_DIR` match your system (see TODO below).
- Use the Welcome Wizard or File > Open, and select c, and chose to _Open as Project_.
...
**TODO / Caveat**: this initial `CMakeLists.txt` is mostly hand-generated by parsing the arduino-cli
build files using find/set/grep. TBD is to create a small python generator that will automatically
write or update that file. Actual build and deploying via CMake is out of the scope of this project,
instead I'd hopw arduino-cli will get that support later.
...
There's an equivalent `_compile_and_monitor.bat` for CMD.

Note that you'll want to customize this script to match your serial port.
